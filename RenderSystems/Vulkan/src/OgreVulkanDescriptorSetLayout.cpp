...
// VulkanDescriptorSetLayout::VulkanDescriptorSetLayout(Device &device, const std::vector<ShaderResource> &resource_set) :
//     device{device}
// {
...
// 	{
// 		throw VulkanException{result, "Cannot create VulkanDescriptorSetLayout"};
// 	}
// }
...
VulkanDescriptorSetLayout::VulkanDescriptorSetLayout(
    ...
    const FastArray<VkDescriptorSetLayoutBinding> &descriptorSetLayoutBindings ) :
    handle( descriptorSetLayout )
{
    FastArray<VkDescriptorSetLayoutBinding>::const_iterator itor = descriptorSetLayoutBindings.begin();
    FastArray<VkDescriptorSetLayoutBinding>::const_iterator end =
        descriptorSetLayoutBindings.end();

    while( itor != end )
    {
        const VkDescriptorSetLayoutBinding binding = *itor;

        bindings.push_back( binding );
        // Store mapping between binding and the binding point
        bindings_lookup.emplace( binding.binding, binding );
        ++itor;
    }
}
...
VkDescriptorSetLayout VulkanDescriptorSetLayout::get_handle() const
{
	return handle;
}

const std::vector<VkDescriptorSetLayoutBinding> &VulkanDescriptorSetLayout::get_bindings() const
{
	return bindings;
}

std::unique_ptr<VkDescriptorSetLayoutBinding> VulkanDescriptorSetLayout::get_layout_binding(uint32_t binding_index) const
{
...
std::unique_ptr<VkDescriptorSetLayoutBinding> VulkanDescriptorSetLayout::get_layout_binding(const std::string &name) const
{
