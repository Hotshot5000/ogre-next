...
{
    const std::vector<VkDescriptorSetLayoutBinding> &bindings = descriptor_set_layout.get_bindings();

	std::map<VkDescriptorType, std::uint32_t> descriptor_type_counts;

	// Count each type of descriptor set
	for ( const std::vector<VkDescriptorSetLayoutBinding>::value_type &binding : bindings)
	{
    ...
    std::vector<struct VkDescriptorPoolSize>::iterator pool_size_it = pool_sizes.begin();

	// Fill pool size for each descriptor type count multiplied by the pool size
	for ( std::map<VkDescriptorType, std::uint32_t>::value_type &it : descriptor_type_counts)
	{
...
VulkanDescriptorPool::~VulkanDescriptorPool()
{
	...
	for ( std::vector<VkDescriptorPool_T *>::value_type pool : pools)
	{
...
const VulkanDescriptorSetLayout &VulkanDescriptorPool::get_descriptor_set_layout() const
{
	assert(descriptor_set_layout && "Descriptor set layout is invalid");
	return *descriptor_set_layout;
}

void VulkanDescriptorPool::set_descriptor_set_layout(const VulkanDescriptorSetLayout &set_layout)
{
...
VkDescriptorSet VulkanDescriptorPool::allocate()
{
    ...
    VkResult result = vkAllocateDescriptorSets( device, &allocInfo, &handle );
...
VkResult VulkanDescriptorPool::free(VkDescriptorSet descriptor_set)
{
	// Get the pool index of the descriptor set
    std::unordered_map<VkDescriptorSet_T *, unsigned>::iterator it = set_pool_mapping.find(
        descriptor_set );
    ...
    unsigned int desc_pool_index = it->second;

	// Free descriptor set from the pool
	vkFreeDescriptorSets(device, pools[desc_pool_index], 1, &descriptor_set);
...
std::uint32_t VulkanDescriptorPool::find_available_pool(std::uint32_t search_index)
{
	// Create a new pool
	if (pools.size() <= search_index)
	{
        ...
        VkResult result = vkCreateDescriptorPool( device, &create_info, nullptr, &handle );
