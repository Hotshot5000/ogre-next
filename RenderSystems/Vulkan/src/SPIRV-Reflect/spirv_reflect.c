...
static SpvReflectResult IntersectSortedUint32( const uint32_t *p_arr0, size_t arr0_size,
                                               const uint32_t *p_arr1, size_t arr1_size,
...
static SpvReflectResult ReadStr( Parser *p_parser, uint32_t word_offset, uint32_t word_index,
                                 uint32_t word_count, uint32_t *p_buf_size, char *p_buf )
{
...
static SpvReflectResult ParseFunction( Parser *p_parser, Node *p_func_node, Function *p_func,
                                       size_t first_label_index )
{
    ...
    if( p_func->accessed_ptr_count > 0 )
    {
        p_func->accessed_ptrs =
            (uint32_t *)calloc( p_func->accessed_ptr_count, sizeof( *( p_func->accessed_ptrs ) ) );
    ...
    if( p_func->accessed_ptr_count > 0 )
    {
        qsort( p_func->accessed_ptrs, p_func->accessed_ptr_count, sizeof( *( p_func->accessed_ptrs ) ),
               SortCompareUint32 );
    }
    p_func->accessed_ptr_count =
        (uint32_t)DedupSortedUint32( p_func->accessed_ptrs, p_func->accessed_ptr_count );
...
static SpvReflectResult ParseFunctions( Parser *p_parser )
{
    ...
    if( IsNotNull( p_parser ) && IsNotNull( p_parser->spirv_code ) && IsNotNull( p_parser->nodes ) )
    {
        ...
        p_parser->functions =
            (Function *)calloc( p_parser->function_count, sizeof( *( p_parser->functions ) ) );
        ...
        for( size_t i = 0; i < p_parser->function_count; ++i )
        {
            ...
            p_func->callee_ptrs =
                (Function **)calloc( p_func->callee_count, sizeof( *( p_func->callee_ptrs ) ) );
    ...
    if( IsNotNull( p_parser ) && IsNotNull( p_parser->spirv_code ) && IsNotNull( p_parser->nodes ) )
    {
        ...
        for( uint32_t i = 0; i < p_parser->node_count; ++i )
        {
            ...
            p_node->member_decorations =
                (Decorations *)calloc( p_node->member_count, sizeof( *( p_node->member_decorations ) ) );
...
static SpvReflectResult EnumerateAllUniforms( SpvReflectShaderModule *p_module, size_t *p_uniform_count,
...
static SpvReflectResult ParseType( Parser *p_parser, Node *p_node,
                                   ...
                                   SpvReflectShaderModule *p_module, SpvReflectTypeDescription *p_type )
{
    ...
    if( p_node->member_count > 0 )
    {
        ...
        p_type->members =
            (SpvReflectTypeDescription *)calloc( p_type->member_count, sizeof( *( p_type->members ) ) );
    ...
    for( size_t i = 0; i < p_parser->node_count; ++i )
    {
        ...
        // This is a workaround for: https://github.com/KhronosGroup/glslang/issues/1096
        {
            const uint32_t resource_mask =
                SPV_REFLECT_TYPE_FLAG_EXTERNAL_SAMPLED_IMAGE | SPV_REFLECT_TYPE_FLAG_EXTERNAL_IMAGE;
...
static SpvReflectResult ParseDescriptorBlockVariable( Parser *p_parser, SpvReflectShaderModule *p_module,
        ...
        for( uint32_t member_index = 0; member_index < p_type->member_count; ++member_index )
        {
            ...
            if( is_struct )
            {
                SpvReflectResult result =
                    ParseDescriptorBlockVariable( p_parser, p_module, p_member_type, p_member_var );
                if( result != SPV_REFLECT_RESULT_SUCCESS )
                {
                    return result;
                }
...
static SpvReflectResult ParseDescriptorBlockVariableSizes( Parser *p_parser,
                                                           ...
                                                           bool is_parent_root, bool is_parent_aos,
                                                           bool is_parent_rta,
                                                           SpvReflectBlockVariable *p_var )
{
    ...
    for( uint32_t member_index = 0; member_index < p_var->member_count; ++member_index )
    {
        ...
        else
        {
            p_member_var->absolute_offset =
                is_parent_aos ? 0 : p_member_var->offset + p_var->absolute_offset;
        }
    }
    ...
    for( uint32_t member_index = 0; member_index < p_var->member_count; ++member_index )
    {
        ...
        case SpvOpTypeMatrix:
        {
            ...
            {
                p_member_var->size =
                    p_member_var->numeric.matrix.column_count * p_member_var->numeric.matrix.stride;
            }
            else if( p_member_var->decoration_flags & SPV_REFLECT_DECORATION_ROW_MAJOR )
            {
                p_member_var->size =
                    p_member_var->numeric.matrix.row_count * p_member_var->numeric.matrix.stride;
            }
        }
        break;
        ...
        case SpvOpTypeRuntimeArray:
        {
            ...
            if( is_struct )
            {
                SpvReflectResult result = ParseDescriptorBlockVariableSizes( p_parser, p_module, false,
                                                                             true, true, p_member_var );
                if( result != SPV_REFLECT_RESULT_SUCCESS )
                {
                    return result;
                }
    ...
    if( p_var->member_count > 0 )
    {
        ...
        p_member_var->padded_size =
            RoundUp( p_member_var->offset + p_member_var->size, SPIRV_DATA_ALIGNMENT ) -
            p_member_var->offset;
        if( p_member_var->size > p_member_var->padded_size )
        {
            p_member_var->size = p_member_var->padded_size;
                                     ...
                                     SpvReflectFormat *p_format )
{
    ...
    if( p_type->type_flags & SPV_REFLECT_TYPE_FLAG_VECTOR )
    {
        ...
        else if( p_type->type_flags & ( SPV_REFLECT_TYPE_FLAG_INT | SPV_REFLECT_TYPE_FLAG_BOOL ) )
        {
            ...
            case 3:
                *p_format =
                    signedness ? SPV_REFLECT_FORMAT_R32G32B32_SINT : SPV_REFLECT_FORMAT_R32G32B32_UINT;
                break;
            case 4:
...
static SpvReflectResult ParseInterfaceVariable(
    Parser *p_parser, const Decorations *p_type_node_decorations, SpvReflectShaderModule *p_module,
    SpvReflectTypeDescription *p_type, SpvReflectInterfaceVariable *p_var, bool *p_has_built_in )
{
    Node *p_type_node = FindNode( p_parser, p_type->id );
    if( IsNull( p_type_node ) )
    {
                                                 ...
                                                 uint32_t *io_vars )
{
    ...
    for( size_t i = 0; i < io_var_count; ++i )
    {
        ...
        SpvReflectResult result = ParseInterfaceVariable( p_parser, &p_type_node->decorations, p_module,
...
static SpvReflectResult TraverseCallGraph( Parser *p_parser, Function *p_func, size_t *p_func_count,
...
static SpvReflectResult ParseStaticallyUsedResources( Parser *p_parser, SpvReflectShaderModule *p_module,
                                                      SpvReflectEntryPoint *p_entry,
                                                      size_t uniform_count, uint32_t *uniforms,
                                                      ...
                                                      uint32_t *push_constants )
{
    ...
    //
    SpvReflectResult result0 =
        IntersectSortedUint32( used_variables, used_variable_count, uniforms, uniform_count,
                               &p_entry->used_uniforms, &used_uniform_count );

    size_t used_push_constant_count = 0;
    //
    SpvReflectResult result1 =
        IntersectSortedUint32( used_variables, used_variable_count, push_constants, push_constant_count,
...
static SpvReflectResult ParseEntryPoints( Parser *p_parser, SpvReflectShaderModule *p_module )
{
    ...
    for( size_t i = 0; entry_point_index < p_parser->entry_point_count && i < p_parser->node_count; ++i )
    {
        ...
        {
            interface_variables =
                (uint32_t *)calloc( interface_variable_count, sizeof( *( interface_variables ) ) );
        ...
        result = ParseInterfaceVariables( p_parser, p_module, p_entry_point, interface_variable_count,
                                          interface_variables );
        if( result != SPV_REFLECT_RESULT_SUCCESS )
        {
            return result;
    ...
    const SpvReflectShaderModule *p_module, const char *entry_point, SpvReflectResult *p_result )
{
    ...
    if( IsNotNull( p_module ) )
    {
        ...
        {
            bool found =
                SearchSortedUint32( p_entry->used_push_constants, p_entry->used_push_constant_count,
                                    p_module->push_constant_blocks[i].spirv_id );
            if( found )
            {
...
SpvReflectResult spvReflectChangeDescriptorBindingNumber(
    SpvReflectShaderModule *p_module, const SpvReflectDescriptorBinding *p_descriptor_binding,
    uint32_t new_binding_number, uint32_t optional_new_set_number )
{
    return spvReflectChangeDescriptorBindingNumbers( p_module, p_descriptor_binding, new_binding_number,
...
SpvReflectResult spvReflectChangeInputVariableLocation(
    SpvReflectShaderModule *p_module, const SpvReflectInterfaceVariable *p_input_variable,
    uint32_t new_location )
{
    if( IsNull( p_module ) )
    {
...
SpvReflectResult spvReflectChangeOutputVariableLocation(
    SpvReflectShaderModule *p_module, const SpvReflectInterfaceVariable *p_output_variable,
    uint32_t new_location )
{
    if( IsNull( p_module ) )
    {
