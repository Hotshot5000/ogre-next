    ...
    uint tid [[thread_position_in_grid]] )
{
    ...
    for( uint idx = 0u; idx < numInstances; ++idx )
    {
        ...
        const float lodRadius = max( src.lodBoundsCenterRadius.w, 1e-6f );
        float projectedDiameterPixels = INFINITY;
        if( cullParams.cullOptions.w > 0.0f )
        {
            const float depth = max( dot( toLodBounds, cullParams.cameraForwardAndNear.xyz ), 1e-4f );
            projectedDiameterPixels = ( lodRadius * 2.0f ) / ( depth * cullParams.cullOptions.w );
        }

        const bool tierVisible = src.tier == 2u ? projectedDiameterPixels < 8.0f :
                                 ( src.tier == 1u ? projectedDiameterPixels >= 8.0f &&
                                                        projectedDiameterPixels < 32.0f :
                                                   projectedDiameterPixels >= 32.0f );
        const bool active = src.active != 0u && distanceVisible && coneVisible && tierVisible;
        if( !active )
            continue;
