...
@foreach( num_textures, n )
	layout(binding=@n) uniform sampler2DArray textureMaps@n;@end

@property( !hlms_enable_cubemaps_auto )
	@property( use_envprobe_map )layout(binding=1) uniform samplerCube		texEnvProbeMap;@end
@else
	@property( !hlms_cubemaps_use_dpm )
		@property( use_envprobe_map )layout(binding=1)uniform samplerCubeArray	texEnvProbeMap;@end
	@else
		@property( use_envprobe_map )layout(binding=1)uniform sampler2DArray	texEnvProbeMap;@end
